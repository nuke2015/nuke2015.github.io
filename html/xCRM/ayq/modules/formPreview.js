...
layui.config({
    base: '/ayq/modules/'
}).define(['layer', 'laytpl', 'element', 'form', 'slider', 'laydate', 'rate', 'colorpicker', 'layedit', 'carousel', 'upload', 'formField', "numberInput", "fengzi", "cityarea", "iconPicker", "cron", "labelGeneration"], function(exports) {
    var $ = layui.jquery,
        layer = layui.layer,
        laytpl = layui.laytpl,
        setter = layui.cache,
        ...
        TPL_SUBMIT = ['<div class="layui-form-item">', '<div class="layui-input-block">', '<button type="submit" class="layui-btn" lay-submit="" lay-filter="demo1">提交</button>', '<button type="reset" class="layui-btn layui-btn-primary">重置</button><a href="javascript:history.go(-1)" class="layui-btn layui-btn-primary">返回</a>', '</div>', '</div>'].join('')
        //外部接口
        ,
        formPreview = {
        ...
        thisIns = function() {
            ...
            return {
                ...
                getpcaObjects: function() {
                    return pcaObjects || null;
                },
                getImages: function() {
                    return images || null;
                },
        ...
        Class = function(options) {
            ...
            that.config = $.extend({}, that.config, formPreview.config, options);
            that.render();
        };
    /* 此方法最后一道 commom.js 中 */
    ...
    Class.prototype.components = {
        ...
        cityarea: {
            ...
            render: function(json, selected) {
                ...
                _html += '<div class="layui-input-inline">'.format(json.width, json.labelWidth);
                _html += '<select name="{0}" id="{0}" lay-filter="{0}" class="layui-select"></select>'.format(json.tag + json.id + '_province');
                _html += '</div>';

                _html += '<div class="layui-input-inline">'.format(json.width, json.labelWidth);
                _html += '<select name="{0}" id="{0}" lay-filter="{0}" class="layui-select" ></select>'.format(json.tag + json.id + '_city');
                _html += '</div>';
                
                _html += '<div class="layui-input-inline">'.format(json.width, json.labelWidth);
                _html += '<select name="{0}" id="{0}" lay-filter="{0}" class="layui-select"></select>'.format(json.tag + json.id + '_area');
                _html += '</div>';
        ...
        fengzi: {
            ...
            render: function(json, selected) {
                ...
                _html += '<select name="{0}" id="{0}" lay-filter="{0}"><option>empty</option></select>'.format(json.tag + json.id);
                _html += '</div>';
                // if(selected){
                //  _html +='<div class="widget-view-action"><i class="layui-icon layui-icon-file"></i><i class="layui-icon layui-icon-delete"></i></div><div class="widget-view-drag"><i class="layui-icon layui-icon-screen-full"></i></div>';
                // }
        ...
        numberInput: {
            ...
            render: function(json, selected) {
                ...
                _html += '<label class="layui-form-label" style="width:{1}px;">{0}:</label>'.format(json.label, json.labelWidth);
                _html += '<div class="layui-input-block" style="width:calc({0} - {1}px);margin-left: {1}px;">'.format(json.width, json.labelWidth);
                _html += '<input name="{0}" id="{9}" value="{1}" placeholder="{3}" class="layui-input{5}" lay-vertype="tips" min="{6}" max="{7}" step="{8}"  {4} style="width:{2}">'.format(json.id, json.defaultValue ? json.defaultValue : '0', json.width, json.placeholder, _disabled, _disabledClass, json.minValue, json.maxValue, json.stepValue, json.tag + json.id);
                _html += '</div>';
                // if(selected){
                //  _html +='<div class="widget-view-action"><i class="layui-icon layui-icon-file"></i><i class="layui-icon layui-icon-delete"></i></div><div class="widget-view-drag"><i class="layui-icon layui-icon-screen-full"></i></div>';
                // }
        ...
        iconPicker: {
            ...
            render: function(json, selected) {
                ...
                _html += '<label class="layui-form-label {0}" style="width: {3}px"><span style="color:red;">{2}</span>{1}:</label>'.format(json.required ? 'layui-form-required' : '', json.label, json.required ? '*' : '', json.labelWidth);
                _html += '<div class="layui-input-block" style="width:calc({0} - {1}px);margin-left: {1}px;">'.format(json.width, json.labelWidth);
                _html += '<input type="cronExpression" name="{0}" id="{5}" value="{1}" lay-verify="{6}" placeholder="{2}" class="layui-input{4}" lay-filter="iconPicker" lay-vertype="tips" {3}>'.format(json.id, json.defaultValue ? json.defaultValue : '', json.placeholder, _disabled, _disabledClass, json.tag + json.id, _required);
                _html += '</div>';
                _html += '</div>';
                return _html;
            },
            update: function(json, value) {
                ...
                var _html = '';
                //重绘设计区改id下的所有元素
                _html += '<input name="{0}" value="{1}" id="{6}" placeholder="{3}" class="layui-input{5}" lay-filter="{6}" lay-vertype="tips" {4} style="width:{2}">'.format(json.id, value ? value : '', json.width, json.placeholder, _disabled, _disabledClass, json.tag + json.id);
                $('#' + json.id + ' .layui-input-block').append(_html);
                iconPicker.render({
                    // 选择器，推荐使用input
                    elem: '#' + json.tag + json.id,
        ...
        cron: {
            ...
            update: function(json, value) {
                ...
                var _html = '';
                //重绘设计区改id下的所有元素
                _html += '<input name="{0}" value="{1}" id="{6}" placeholder="{3}" class="layui-input{5}" lay-filter="iconPicker" lay-vertype="tips" {4} style="width:{2}">'.format(json.id, value ? value : '', json.width, json.placeholder, _disabled, _disabledClass, json.tag + json.id);
                if (!json.disabled) {
                    var _width = json.width.replace(/[^\d]/g, '');
                    if ('' != _width) {
                        _width = 100 - parseInt(_width);
        ...
        image: {
            ...
            render: function(json, selected) {
                ...
                title=json.title||'图片上传';
                _html += '<button type="button" class="layui-btn" id="{0}">{1}</button>'.format(json.tag + json.id,title);
        ...
        html: {
             ...
             * */
            render: function(json, selected) {
                console.log("html render", json, selected);
                if (selected === undefined) {
                    selected = false;
                }
                var _html = '<div id="{0}" class="layui-form-item {2}" style="width: {4}"  data-id="{0}" data-tag="{1}" data-index="{3}">'.format(json.id, json.tag, selected ? 'active' : '', json.index, json.width);
                _html += '<label class="layui-form-label">{0}:</label>'.format(json.label, json.width);
                _html += '<div>';
                _html += '<div id="{0}">{1}</div>'.format(json.tag + json.id, json.html);
                _html += '</div>';
                _html += '</div>';
                return _html;
            },
            update: function(json, value) {
                console.log("ayq html update");
            },
            /* 获取对象 */
            jsonData: function(id, index, columncount) {
                //分配一个新的ID
        ...
        pca: {
             ...
             * */
            render: function(json, selected) {
                console.log("pca render", json, selected);
                if (selected === undefined) {
                    selected = false;
                }
                ...
                _html += '<div class="layui-input-block">';
                _html += '<div class="layui-form" id="{0}" lay-filter="{0}" >{1}</div>'.format(json.tag + json.id, '<div class="layui-input-inline"> <div>  <div class="layui-input-inline"> <select name="provice" id="pca_province" class="layui-select"> <option value="">省份</option> </select> <select name="city" id="pca_city" class="layui-select"> <option value="">市级</option> </select> <select name="area" id="pca_area" class="layui-select"> <option value="">区县</option> </select> </div> <div class="cb"></div> </div> </div>');
                _html += '</div>';
                // if(selected){
                //  _html +='<div class="widget-view-action"><i class="layui-icon layui-icon-file"></i><i class="layui-icon layui-icon-delete"></i></div><div class="widget-view-drag"><i class="layui-icon layui-icon-screen-full"></i></div>';
                // }
            ...
            update: function(json, value) {
                console.log("pca update");
                var e = pcaObjects[json.id];
                e.setValue(value);
            },
            /* 获取对象 */
            jsonData: function(id, index, columncount) {
    ...
    Class.prototype.setFormData = function(json) {
        ...
        for (let key in json) {
            ...
            if (item.tag === 'editor' || item.tag === 'rate' || item.tag === 'slider' || item.tag === 'labelGeneration' || item.tag === 'sign' || item.tag === 'iconPicker' || item.tag === 'cron' || item.tag === 'colorpicker') {
    ...
    Class.prototype.renderComponents = function(jsondata, elem) {
        ...
        $.each(jsondata, function(index, item) {
            ...
            if (item.tag === 'grid') {
            ...
            } else if (item.tag === 'fengzi') {
                //定义初始值
                fengzi.render({
                    elem: '#' + item.tag + item.id
                });
            } else if (item.tag === 'cityarea') {
                // 省市区
                cityarea.render({
                    elem: item.tag + item.id,
                    item: item
                ...
                if (item.dateRangeDefaultValue !== null && item.dateRangeDefaultValue !== "" && item.dateRangeDefaultValue !== undefined) {
            ...
            } else if (item.tag === 'pca') {
                console.log('pca render =init', item);
                api_pca.init(0, 0, 0);
                var index = item.tag + item.id;
                console.info(index);
                form.render();
            } else if (item.tag === 'html') {
                console.log('html render =init',item);
                // 回传事件绑定
                item.bind(item);
                // 省市区
            ...
            for (let i = 0; i < options.data.length; i++) {
                if (options.data[i].tag === 'grid') {
                    for (let j = 0; j < options.data[i].columns.length; j++) {
                        for (let k = 0; k < options.data[i].columns[j].list.length; k++) {
                            if (options.data[i].columns[j].list[k].tag === 'select') {
                                $('#' + options.data[i].columns[j].list[k].id + ' .layui-input-block div.layui-unselect.layui-form-select').css({
                                    width: '{0}'.format(options.data[i].columns[j].list[k].width)
                                });
                            }
                        }
                    }
                    ...
                    if (options.data[i].tag === 'select') {
                        $('#' + options.data[i].id + ' .layui-input-block div.layui-unselect.layui-form-select').css({
                            width: '{0}'.format(options.data[i].width)
                        });
                    }
                }
            }
