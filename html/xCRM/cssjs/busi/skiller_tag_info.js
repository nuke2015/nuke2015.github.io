...
layui.config({
    ...
    form.on('submit(demo1)', function(data) {
        ...
        if (id > 0) {
            json.id = id;
            json.methodName = "skiller_tagEdit";
        } else {
            json.methodName = "skiller_tagAdd";
        }
        json.table = "bestphp_skiller_tag";
        // 读图片
        json.pic = fengform.upload_get("pic");
        api_crm(json, function(res) {
            layer.msg(res.msg, function() {
                location.href = "/html/skiller_tag_list.html";
            });
        }, function(res) {
            console.log(res);
        });
