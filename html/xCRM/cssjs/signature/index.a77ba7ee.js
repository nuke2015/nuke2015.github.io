...
var f = function() {
    function r(a, o) {
        var t = this;
        F(this, r), this.canvas = {}, this.ctx = {}, this.width = 320, this.height = 200, this.scale = window.devicePixelRatio || 1, this.color = "black", this.bgColor = "", this.canDraw = !1, this.openSmooth = !0, this.minWidth = 2, this.maxWidth = 6, this.minSpeed = 1.5, this.maxWidthDiffRate = 20, this.points = [], this.canAddHistory = !0, this.historyList = [], this.maxHistoryLength = 20, this.onStart = function() {}, this.onEnd = function() {}, this.addListener = function() {
            t.removeListener(), t.canvas.style.touchAction = "none", "ontouchstart" in window || navigator.maxTouchPoints ? (t.canvas.addEventListener("touchstart", t.onDrawStart), t.canvas.addEventListener("touchmove", t.onDrawMove), document.addEventListener("touchcancel", t.onDrawEnd), document.addEventListener("touchend", t.onDrawEnd)) : (t.canvas.addEventListener("mousedown", t.onDrawStart), t.canvas.addEventListener("mousemove", t.onDrawMove), document.addEventListener("mouseup", t.onDrawEnd))
        }, this.removeListener = function() {
            t.canvas.style.touchAction = "auto", t.canvas.removeEventListener("touchstart", t.onDrawStart), t.canvas.removeEventListener("touchmove", t.onDrawMove), document.removeEventListener("touchend", t.onDrawEnd), document.removeEventListener("touchcancel", t.onDrawEnd), t.canvas.removeEventListener("mousedown", t.onDrawStart), t.canvas.removeEventListener("mousemove", t.onDrawMove), document.removeEventListener("mouseup", t.onDrawEnd)
        }, this.onDrawStart = function(e) {
            e.preventDefault(), t.canDraw = !0, t.canAddHistory = !0, t.ctx.strokeStyle = t.color, t.initPoint(e), t.onStart && t.onStart(e)
            ...
            if (!(i && i.t === n)) {
                ...
                if (!(i && i.x === d && i.y === c)) {
                    ...
                    if (t.openSmooth && i) {
                        ...
                        if (u.distance = Math.sqrt(Math.pow(u.x - i.x, 2) + Math.pow(u.y - i.y, 2)), u.speed = u.distance / (u.t - i.t || .1), u.lineWidth = t.getLineWidth(u.speed), w && w.lineWidth && i.lineWidth) {
            ...
            var i = n.x - e.x,
                s = n.y - e.y;
            if (Math.abs(i) + Math.abs(s) <= t.scale ? (n.lastX1 = n.lastX2 = e.x + i * .5, n.lastY1 = n.lastY2 = e.y + s * .5) : (n.lastX1 = e.x + i * .3, n.lastY1 = e.y + s * .3, n.lastX2 = e.x + i * .7, n.lastY2 = e.y + s * .7), n.perLineWidth = (e.lineWidth + n.lineWidth) / 2, typeof e.lastX1 == "number") {
                if (t.drawCurveLine(e.lastX2, e.lastY2, e.x, e.y, n.lastX1, n.lastY1, n.perLineWidth), e.isFirstPoint || e.lastX1 === e.lastX2 && e.lastY1 === e.lastY2) return;
            ...
            n.lastX = e.x + (n.x - e.x) * .5, n.lastY = e.y + (n.y - e.y) * .5, typeof e.lastX == "number" && t.drawCurveLine(e.lastX, e.lastY, e.x, e.y, n.lastX, n.lastY, t.maxWidth)
        }, this.drawCurveLine = function(e, n, i, s, h, d, c) {
            t.ctx.lineWidth = Number(c.toFixed(1)), t.ctx.beginPath(), t.ctx.moveTo(Number(e.toFixed(1)), Number(n.toFixed(1))), t.ctx.quadraticCurveTo(Number(i.toFixed(1)), Number(s.toFixed(1)), Number(h.toFixed(1)), Number(d.toFixed(1))), t.ctx.stroke()
        }, this.drawTrapezoid = function(e, n, i, s) {
            t.ctx.beginPath(), t.ctx.moveTo(Number(e.x.toFixed(1)), Number(e.y.toFixed(1))), t.ctx.lineTo(Number(n.x.toFixed(1)), Number(n.y.toFixed(1))), t.ctx.lineTo(Number(i.x.toFixed(1)), Number(i.y.toFixed(1))), t.ctx.lineTo(Number(s.x.toFixed(1)), Number(s.y.toFixed(1))), t.ctx.fillStyle = t.color, t.ctx.fill()
            ...
            !t.maxHistoryLength || !t.canAddHistory || (t.canAddHistory = !1, t.historyList.push(t.canvas.toDataURL()), t.historyList = t.historyList.slice(-t.maxHistoryLength))
        }, this.clear = function() {
            ...
            return h.rotate(e * Math.PI / 180), e === 90 ? h.drawImage(t.canvas, 0, -s, i, s) : e === -90 ? h.drawImage(t.canvas, -i, 0, i, s) : e === 180 && h.drawImage(t.canvas, -i, -s, i, s), n
        }, this.init(a, o)
    }
    return N(r, [{
        key: "init",
        value: function(o) {
            ...
            !o || (this.canvas = o, this.ctx = o.getContext("2d"), this.width = t.width || o.clientWidth || this.width, this.height = t.height || o.clientHeight || this.height, this.scale = t.scale || this.scale, this.color = t.color || this.color, this.bgColor = t.bgColor || this.bgColor, this.openSmooth = t.openSmooth || this.openSmooth, this.minWidth = t.minWidth || this.minWidth, this.maxWidth = t.maxWidth || this.maxWidth, this.minSpeed = t.minSpeed || this.minSpeed, this.maxWidthDiffRate = t.maxWidthDiffRate || this.maxWidthDiffRate, this.maxHistoryLength = t.maxHistoryLength || this.maxHistoryLength, this.onStart = t.onStart, this.onEnd = t.onEnd, this.scale > 0 && (this.canvas.height = this.height * this.scale, this.canvas.width = this.width * this.scale, this.scale !== 1 && (this.canvas.style.width = this.width + "px", this.canvas.style.height = this.height + "px", this.ctx.scale(this.scale, this.scale))), this.ctx.lineCap = "round", this.drawBgColor(), this.addListener())
        }
    }]), r
}();
var D = (r, a) => {
...
function I(r, a, o, t, e, n) {
    return v(), g("div", k, [l("div", R, [l("button", {
        onClick: a[99] || (a[99] = (...i) => api.submity && api.submity(...i))
    }, "确认")]), M, X])
}
var H = D(U, [
    ["render", I]
