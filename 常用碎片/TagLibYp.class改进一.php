 ...
 *
 * @package      	MAXHOM
 * @author          dengweicai QQ:1519287158 
 * @copyright     	Copyright (c) 2012-2022  (http://www.maxhom.com)
 * @version        	驰骋网络 v1.0 
 */
if(defined('APP_NAME')!='Maxhom' && !defined("MAXHOM"))  exit("Access Denied");
class TagLibYp extends TagLib
{
	protected $tags   =  array(
        ...
        'get'=>array('attr'=>'table,field,limit,order,where,mod','level'=>3),
		...
		'nav_old' => array('attr'=>'catid,bcid,level,id,class,home,enhomefont','close'=>0),
	...
	public function _nav_old($attr,$content) {
		$tag        = $this->parseXmlAttr($attr,'nav');
		$level = !empty($tag['level'])? intval($tag['level']) : '1';
		$catid = !empty($tag['catid'])? $tag['catid'] : '0';
		$bcid = !empty($tag['bcid'])? intval($tag['bcid']) : '0';
		...
		if($catid && !is_numeric($catid)){
 		...
 		if(is_array($category_arr)){
			...
			import ( '@.ORG.Tree' );
			$tree = new Tree ($array);
			$parsestr = $tree->get_nav_old($catid,$level,$id,$class,$homefont,FALSE,'',$enhomefont,$lang);
		}unset($category_arr);
		return  $parsestr;
	}
	...
	public function _get($attr,$content) {
		...
		$sql = "M('$table')->where($where)->field('$field')->limit('$limit')->order('$order')->select()";
