...
class  ThinkTemplate {
    ...
    protected function compiler($tmplContent){
		...
		if(defined('TMPL_STRIP_SPACE') && TMPL_STRIP_SPACE){
			/* 去除html空格与换行,对cssjs正则会出问题 */
			$tmplContent=str_replace("\n",'',$tmplContent);//清除换行符
			$tmplContent=str_replace("\t",'',$tmplContent);//清除制表符
            $find           = array('~>\s+<~','~>(\s+\n|\r)~');
            $replace        = array('><','>');
    ...
    public function parseTag($tagStr){
		...
		if('$' == $flag){ //解析模板变量 格式 {$varName}
        ...
        }elseif(substr($tagStr,0,3)=='if '){  //开始仿smarty风格;
			...
			return '<?php '.substr($tagStr,4).';?>';  //开始仿smarty风格;
        }elseif('~' == $flag){ // 执行某个函数
            return  '<?php '.$name.';?>';
    ...
    public function parseVar($varStr){
        ...
        if(!empty($varStr)){
            ...
            if('think.' == substr($var,0,6)||'Think.' == substr($var,0,6)){
