...
class LtDbHandle
{
	 ...
	 * 
	 */
	private function fenglog($sqlstr){
		$result=date("Y/m/d H:i:s",time());
		$result.="\r\n";
		
		$result.=$sqlstr."\r\n";
		$result.="\r\n";
		file_put_contents('SQL_LOTUS.log',$result,FILE_APPEND);
	}
	...
	{ 
        ...
        foreach($parameter as $key => $value)
		{
			...
			else if (is_string($value))
			{
				...
				{//扩展sql在in查询下value值为"(1,2,3)"的情况 vincent.li 于2014-6-5
					$replacement[] = $value;
				}
				else if(stripos($value, '+')!==false)
				{//扩展sql在执行字段自加1的情况若'sum=sum+1'  vincent.li 于2014-6-5
					$replacement[] = $value;
				}
				else if(stripos($value, '-')!==false)
				{//扩展sql在执行字段自减1的情况若'sum=sum-1'  vincent.li 于2014-7-14
					$replacement[] = $value;
				}
				else
				{
